...
export class Bundleview {
    ...
    constructor(input, selector) {
        ...
        function getColorMetricsFor(node, onlyNumericAttributes) {
            var leaf = randomLeaf(node);
            var metrics = { default: () => 1 };
            Object.keys(leaf.attributes).forEach(attributeName => {
                if(!onlyNumericAttributes || Number.isFinite(leaf.attributes[attributeName])) {
                    metrics[attributeName] = d => d.attributes[attributeName];
                    walkTree(root, node => {
                        if(!isLeaf(node)) { return; }
                    });
        ...
        var colorMetrics = getMetricsFor(root, false);
        ...
        function colorMetricChanged(value) {
            path.transition()
                .duration(750)
                .call(styleTween, "fill", "red");
        ...
        var path = hierarchicalEdgeBundleEnterElementGroup.append("path")
            ...
            .style('fill', d => isLeaf(d) ? '#6666af' : null);
        var node = path;
