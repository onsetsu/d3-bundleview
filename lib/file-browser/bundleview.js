...
import openContextMenuFor from './../context-menu.js';
...
var bundleviewDiv = d3.select(newWidget(6, 0, 7, 7)).append('div');
    ...
    constructor(root, links) {
        ...
        var hierarchicalEdgeBundleEnterElementGroup = enterElem.append("g")
            ...
            .on('contextmenu', function(d, ...args) {
                openContextMenuFor(new Map([
                    ["open", clickedOnNode.bind(this, d, ...args)],
                    ["log", () => console.log(d)]
                ]));
            });
...
if(false) {
    d3.json("example/flare.json", function(error, root) {
        ...
        new FileTreeBundleview(root, getRandomLinks(root, 300));
    });
} else {
    ServerProxy.glob2('./example', '*.js').then(function(stuff) {
        var numberOfFiles = 0;
        console.log(stuff);
        ...
        new FileTreeBundleview(stuff, getRandomLinks(stuff, 200));
    });
}
